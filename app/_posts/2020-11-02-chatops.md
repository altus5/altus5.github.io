...
すいぶんと前から Slack の Chatbot を作ってみたいなぁ・・・と思いながら、
時間がなくて手を出せていなかったのですが、AWS Chatbot で ChatOps の一端に触れて、
”やる気スイッチ”が入りました。  
せっかくなので、サンプルを動かしてみるところまでやってみたので、ご紹介します。  

## Chatbot のフレームワーク

普段一番使っているのが Slack なので、Slack の Bot を作ります。  
そして、できるだけ仕事で使える ChatOps にしたいので、Chatbot 用のフレームワークを使った方が、高機能化しやすいだろうと思います。

ということで、Chatbot 用のフレームワークを [npm trends](https://www.npmtrends.com/hubot-vs-botbuilder-vs-rasa-nlu-trainer-vs-botkit-vs-botpress) で人気の比較をしてみました。

![npm trends](/images/blog/chatops/npm-trends.png)

Hubot が人気なのかなぁ・・・と想像しながら、検索してみたら、 BotBuilder なんですね。  
そして BotBuilder が Microsoft 製の OSS だということで、開発が止まってしまう心配もないので、これを使うことにします。

## Slack Bot を AWS Lambda で運用したい

BotBuilder は、対応している Chat が 2020年11月時点で、21 種類もあって、SDKが4つの言語用（C# / JavaScript / Python / Java）に用意されています。  
そして、サンプル実装が、大量にあります。

今回は、この中から、最近 Python が人気なので、Python SDK を使って Slack のサンプルを動かすことにします。  
...
コンテナ内に、Web API の URL を実装して、 ngrok 経由でその URL を実行できるようにしておいて、 Slack アプリの Event Subscriptions の Request URL に ngrok の URL を設定すると、 Bot が参加したチャンネルのメッセージを Slack が POST してくれます。
...
起動コマンドを忘れるので、docker-compose にしておきます。
...
deploy コマンドが作成された API Gateway の URL を出力しているので、それをコピーして、
...
いかがだったでしょうか？  
これだけでも、 Slack 起点で、いろんなことができそうです。  
さらに BotBuilder の中身を勉強して、いろいろ試してみようと思います。  

この記事では、AWSにデプロイしてますが、 [Azure Bot Service](https://azure.microsoft.com/ja-jp/services/bot-service/) を使うことで、会話するボットも、簡単に作成できそうな感じです。また時間を見つけて試してみたいです。

みなさんも、 ChatOps にトライしてみませんか？
